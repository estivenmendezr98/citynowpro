...
const ListResult = (props) => {
    ...
    useEffect(() => {
        ...
        if (props.textForFilter !== "") {
            if (props.headquarters) {
                fetchStudentsSearch(
                    props.textForFilter.toLowerCase().replace(/\s+/g, "")
                );
            } else {
                setFilter(
                    data.filter(function (i) {
                        return i.name
                            .toLowerCase()
                            .match(props.textForFilter.toLowerCase());
                    })
                );
            }
        } else {
    ...
    async function fetchStudents(page = 1) {
        if (perPage !== null) {
            ...
            try {
                let infoStudents = await fetchPost(
                    `http://127.0.0.1:8000/infoUser/students?page=${page}`,
                    jsonToPost
                );
    ...
    async function fetchStudentsSearch(toSearch) {
        ...
        try {
            let infoStudents = await fetchPost(
                `http://127.0.0.1:8000/infoUser/students`,
                jsonToPost
            );
            setData(infoStudents.game_users.data);
            setelements(infoStudents.game_users.data);
    ...
    function handlePageClick(data) {
        let selectedPage = props.headquarters
            ? data.selected + 1
            : data.selected;
        let offset = selectedPage * perPage;
        if (props.headquarters) {
