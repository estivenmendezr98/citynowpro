...
const ResultCompetitionsGroup = ({ idForFetch, nameItemClicked }) => {
    ...
    async function fetchData() {
        setisLoaded(true)
        try {
            const intelligence = await fetchApi(`http://127.0.0.1:8000/competences/intelligences/grade/${idForFetch}`)
            getIntelligences(intelligence)
            const styles = await fetchApi(`http://127.0.0.1:8000/competences/styles/grade/${idForFetch}`)
            getStyles(styles)
...
export default ResultCompetitionsGroup