...
const LearningStylesHeadquater = ({ idForFetch, showAllData , nameItemClicked }) => {
    ...
    async function fetchData() {
        try {
            const result = await fetchApi(`http://127.0.0.1:8000/styles/headquarter/${idForFetch}`)
            setJsonApi(result)
            setisLoaded(false)
        } catch (error) {
            setisLoaded(true)
...
export default LearningStylesHeadquater;