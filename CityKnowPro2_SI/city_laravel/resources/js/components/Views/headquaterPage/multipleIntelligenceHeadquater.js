...
const MultipleIntelligencesHeadquater = ({ idForFetch, showAllData, nameItemClicked }) => {
    ...
    async function fetchData() {
        try {
            const result = await fetchApi(`http://127.0.0.1:8000/intelligences/headquarter/${idForFetch}`)
            setJsonApi(result)

            const gamesPlayedInfo = await fetchApi(`http://127.0.0.1:8000/gamesPlayed/headquarter/${idForFetch}/inteligencias`)
...
export default MultipleIntelligencesHeadquater;