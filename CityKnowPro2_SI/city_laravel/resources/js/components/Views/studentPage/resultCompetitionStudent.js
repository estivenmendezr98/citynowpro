...
const ResultCompetitionsStudent = ({ idForFetch, nameItemClicked }) => {
    ...
    async function fetchData() {
        setisLoaded(true)
        try {
            const intelligence = await fetchApi(`http://127.0.0.1:8000/competences/intelligences/${idForFetch}`)
            getIntelligences(intelligence)
            const styles = await fetchApi(`http://127.0.0.1:8000/competences/styles/${idForFetch}`)
            getStyles(styles)
...
export default ResultCompetitionsStudent